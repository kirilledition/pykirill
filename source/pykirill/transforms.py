...
def principal_component_analysis(*args, **kwargs):
    """
    Dummy function that raises an ImportError if scikit-learn is not installed.

    Raises:
        ImportError: If scikit-learn is not installed, provides a helpful error message.
    """
    raise ImportError(
        "scikit-learn is required for PCA functionality. "
        "Please install it using the command: pip install pykirill[pca]"
    )
